...
projectNames = Project.query.with_entities(Project.pnum).distinct()
projectChoices = [(row[0],row[0]) for row in projectNames]  # change
projectResults=list()
for row in projectNames:
    projectDict=row._asdict()
    results.append(projectDict)
projectChoices2 = [(row['pname'],row['pname']) for row in projectResults]

projectNames = Project.query.with_entities(Project.pname).distinct()
projectChoices = [(row[0],row[0]) for row in projectNames]  # change
projectResults=list()
for row in projectNames:
    projectDict=row._asdict()
...
#    dnumber=IntegerField('Project Number', validators=[DataRequired()])
    dnumber = HiddenField("")
...
#  One of many ways to use SelectField or QuerySelectField.  Lots of issues using those fields!!
    pname = SelectField("Project Name", choices=projectChoices2)  # myChoices defined at top
    employee = SelectField("Employee", choices=projectChoices2)
